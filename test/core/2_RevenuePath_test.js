...
context('RevenuePath: Adding New Tiers', function () {
  ...
  before(async () => {
    ...
    const revPath = await reveelFactory.createRevenuePath(tiers, distributionLists, tierLimits, "Konoha Shinobis", false);
    await revPath.wait();
  ...
  it('Add multiple tiers to existing revenue path ', async () => {
    ...
    const distributionLists = [[2600, 4400, 3000], [5000, 5000]];
  ...
  it('Emits event when adding multiple tiers to existing revenue path ', async () => {
    ...
    await expect(revenuePath.addRevenueTier(tiers, distributionLists, previousTierLimits))
      .to.emit(revenuePath, 'RevenueTiersAdded');
  ...
  it('Reverts adding tiers if tier address list is not equal to distribution list length ', async () => {
    ...
    await expect(revenuePath.addRevenueTier(tier, distributionList, previousTierLimit))
  ...
  it('Reverts adding tiers if total tiers address list is not equal to total distributions list ', async () => {
    ...
    await expect(revenuePath.addRevenueTier(tiers, distributionLists, previousTierLimit))
      .to.be.revertedWithCustomError(RevenuePath, 'WalletAndDistributionCountMismatch');
  ...
  it('Reverts adding tier with previousTierLimit less than or equal to the amount already present in the last tier ', async () => {
    ...
    await expect(revenuePath.addRevenueTier(tier, distributionList, previousTierLimit))
      .to.be.revertedWithCustomError(RevenuePath, 'LimitNotGreaterThanTotalDistributed');
  ...
  it('Reverts if adding revenue path to immutable contract ', async () => {
    ...
    const { tiers, distributionLists, tierLimits } = await loadFixture(pathInitializerFixture);
    const revPath = await reveelFactory.createRevenuePath(tiers, distributionLists, tierLimits, "Konoha Shinobis", true);
    await revPath.wait();
    const revPathAddress = (await reveelFactory.getPaths())[0][0];
    ...
    await expect(revenuePath1.addRevenueTier(tier, distributionList, previousTierLimit))
      .to.be.revertedWithCustomError(RevenuePath, 'RevenuePathNotMutable');
  ...
  it(' Fee is introduced if revenue path has greater than one tier ', async () => {
    ...
    const revPath = await reveelFactory.createRevenuePath(tier, distributionList, tierLimits, "Konoha Shinobis 2", false);
    await revPath.wait();
...
context('RevenuePath: Update paths', function () {
  ...
  beforeEach(async () => {
    ...
    const revPath = await reveelFactory.createRevenuePath(tiers, distributionLists, tierLimits, "Konoha Shinobis 3", false);
    await revPath.wait();
    const revPathAddress = (await reveelFactory.getPaths())[0][0];
  ...
  it('Emits event on Update of revenue tier for given tier number ', async () => {
    ...
    await expect(revenuePath.updateRevenueTier(tier, distributionList, newTierLimit, 1)).to.emit(revenuePath, 'RevenueTiersUpdated');
  ...
  it('Reverts for tier number lesser than current tier during tier updates ', async () => {
    ...
    await expect(revenuePath.updateRevenueTier(tier, distributionList, newTierLimit, 1))
      .to.be.revertedWithCustomError(RevenuePath, 'IneligibileTierUpdate');
  ...
  it('Reverts tier update for tier number not added  ', async () => {
    ...
    await expect(revenuePath.updateRevenueTier(tier, distributionList, newTierLimit, 3))
      .to.be.revertedWithCustomError(RevenuePath, 'IneligibileTierUpdate');
  ...
  it('Reverts for tier updates where distribution list and tier address list length are not equal ', async () => {
    ...
    await expect(revenuePath.updateRevenueTier(tier, distributionList, newTierLimit, 2))
      .to.be.revertedWithCustomError(RevenuePath, 'WalletAndDistributionCountMismatch');
  ...
  it('Reverts for erc20 tier update where distribution totalIs not 100% ', async () => {
    ...
    await expect(revenuePath.updateErc20Distrbution(tier, distributionList))
      .to.be.revertedWithCustomError(RevenuePath, 'TotalShareNotHundred');
...
context("RevenuePath: ETH Distribution", function () {
  ...
  it('Emits event on ETH release ', async () => {
    ...
    await expect(revenuePath.release(tracy.address))
      .to.emit(revenuePath, 'PaymentReleased');
  ...
  it('Reverts if user has no pending ETH balance to claim ', async () => {
    ...
    await expect(revenuePath.release(tracy.address))
      .to.be.revertedWithCustomError(RevenuePath, 'InsufficientWithdrawalBalance');
...
context('RevenuePath: ERC20 Distribution', function () {
  ...
  beforeEach(async () => {
    ...
    const revPath = await reveelFactory.createRevenuePath(tiers, distributionLists, tierLimits, "Konoha Shinobis 1", false);
    await revPath.wait();
    const revPathAddress = (await reveelFactory.getPaths())[0][0];
    revenuePath = await RevenuePath.attach(revPathAddress);
  ...
  it('Emits event when ERC20 release is successful ', async () => {
    ...
    await expect(revenuePath.releaseERC20(simpleToken.address, bob.address))
      .to.emit(revenuePath, 'ERC20PaymentReleased');
  ...
  it('Reverts ERC20 release if there is no revenueShare ', async () => {
    ...
    await expect(revenuePath.releaseERC20(simpleToken.address, kim.address))
      .to.revertedWithCustomError(RevenuePath, 'ZeroERC20Shares');
  ...
  it('Reverts ERC20 release if there is no revenueShare ', async () => {
    ...
    await expect(revenuePath.releaseERC20(simpleToken.address, bob.address))
      .to.revertedWithCustomError(RevenuePath, 'NoDueERC20Payment');
