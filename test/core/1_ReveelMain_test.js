...
context("ReveelMain: Admin", function () {
  ...
  it("Emits event for new platform wallet", async()=> {
    ...
    ).to.emit(reveelFactory, "UpdatedPlatformWallet").withArgs(newPlatformWallet);
  ...
  it("Emits event for new platform fee", async()=> {
    ...
    await expect(reveelFactory.setPlatformFee(newFee)
    ).to.emit(reveelFactory, "UpdatedPlatformFee").withArgs(newFee);
  ...
  it("Emits event for new library address", async()=> {
    ...
    await expect(reveelFactory.setLibraryAddress(newlibrary.address))
      .to.emit(reveelFactory, "UpdatedLibraryAddress").withArgs(newlibrary.address);
  ...
  it("Reverts other than owner chaniging platform wallet", async()=> {
    ...
    await expect(reveelFactory.connect(
      bob).setPlatformWallet(newPlatformWallet)).to.be.revertedWith("Ownable: caller is not the owner");
  ...
  it("Reverts other than owner chaniging platform fee", async()=> {
    ...
    await expect(reveelFactory.connect(
      tracy).setPlatformFee(newFee)).to.be.revertedWith("Ownable: caller is not the owner");
  ...
  it("Reverts other than owner chaniging library address", async()=> {
    ...
    await expect(reveelFactory.connect(
      tirtha).setLibraryAddress(newlibrary.address)).to.be.revertedWith("Ownable: caller is not the owner");

  });

  it("Reverts chaniging library address to zero", async()=> {

    await expect(reveelFactory.setLibraryAddress(constants.ZERO_ADDRESS)).to.be.revertedWithCustomError(ReveelMain,'ZeroAddressProvided');

  });

  it("Reverts chaniging platform wallet address to zero", async()=> {

    await expect(reveelFactory.setPlatformWallet(constants.ZERO_ADDRESS)).to.be.revertedWithCustomError(ReveelMain,'ZeroAddressProvided');

  });

  it("Reverts main deployment with zero library address & platformwallet address", async()=> {

    await expect(ReveelMain.deploy(constants.ZERO_ADDRESS, 1000, constants.ZERO_ADDRESS)).to.be.revertedWithCustomError(ReveelMain,'ZeroAddressProvided');
...
context("ReveelMain: Path Creation", function () {
  ...
  it("Create Revenue Path", async () => {
    ...
    // const revPathContract = await RevenuePath.attach(revPathAddress);
    // const data = await revPathContract.name();
  ...
  it("Reverts if tier addres lists and distribution lists dont' have same length not equal", async()=>{
   ...
   await expect(reveelFactory.createRevenuePath(
      tiers, distributionLists, tierLimits, "Music OGs", true))
  ...
  it("Reverts single tier revenue path creation if share not equal to 100% ", async () => {
    ...
    await expect ( reveelFactory.createRevenuePath(tier, distributionList, tierLimit, "Music OGs", true))
  ...
  it("Reverts multi tier revenue path creation if share not equal to 100% ", async () => {
    ...
    await expect ( reveelFactory.createRevenuePath(tiers, distributionLists, tierLimits, "Music OGs", true))
    .to.be.revertedWithCustomError(RevenuePath,'TotalShareNotHundred');
  });
