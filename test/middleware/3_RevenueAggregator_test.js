...
before(async () => {
    ...
    const revPath1 = await reveelFactory.createRevenuePath(tiers, distributionLists, tierLimits, "Konoha Shinobis 1", false);
    await revPath1.wait();

    const revPath2 = await reveelFactory.createRevenuePath([[bob.address, tracy.address, alex.address, kim.address]],
        [[2200, 2800, 3000, 2000]], [], "Konoha Shinobis 2", true);
    await revPath2.wait();

    const revPath3 = await reveelFactory.createRevenuePath(tiers, distributionLists, tierLimits, "Konoha Shinobis 3", true);
    await revPath3.wait();
...
context('Multi Withdrawal: Aggregator', function () {
    ...
    it('Emits event for withdraw of ERC20 across paths', async () => {
        ...
        await expect(revenueAggregator.withdrawPathErc20(paths, bob.address, simpleToken.address))
        .to.emit(revenueAggregator, 'WithdrawStatus');
    ...
    it('Reverts withdraw ERC20 if zero address path is passed ', async () => {
        ...
        await expect(revenueAggregator.withdrawPathErc20(
    ...
    it('Reverts withdraw ETH if zero address path is passed ', async () => {
        ...
        await expect(revenueAggregator.withdrawPathEth(
            [constants.ZERO_ADDRESS,revenuePath1.address], 
            bob.address))
        .to.revertedWith('ZERO_ADDRESS_CAN_NOT_BE_CONTRACT');
